 ...
 */
object AppWeekStaticsAnalyseTest {
  def main(args: Array[String]) {
    val date: Date = new Date

    val strTime = "2016-12-12 03:11:11" //当前时间
    ...
    val beforeEightDate = DateUtils2.getBeforeNumDay(date1, -8) //获取当前时间前八天


    val beforeOneYear = DateUtils2.splitDate(beforeOneDate)("year") //获取当前时间前一天所属年的Code
    val beforeTwoYear = DateUtils2.splitDate(beforeEightDate)("year") //获取当前时间前八天所属年的Code

    val weekCode = DateUtils2.splitDate(beforeOneDate)("week") //获取当前时间前一天时间所属周的Code
    val beforeOneWeek = DateUtils2.getNowWeekOfYear(beforeOneDate) //获取当前时间前一天时间所属的周
    val beforeTwoWeek = DateUtils2.getNowWeekOfYear(beforeEightDate) //获取当前时间前八天所属的周

    val day = DateUtils2.getDateDay(beforeOneDate)

    val firstWhereCondition = "year(ctime) = " + beforeOneYear + " and weekofyear(ctime) = " + beforeOneWeek
    val secondWhereCondition = "year(ctime) =" + beforeTwoYear + " and weekofyear(ctime) = " + beforeTwoWeek
    ...
    val appWeekProfileBankDF = dataFrameReader.option("table", "app").load().where(firstWhereCondition)
    ...
    val newUserDF = sparkSession.sql("select '" + nowDate + "' ctime, " +
    ...
    val weekAppUserStatsDF = sparkSession.sql("select '" +
      ...
      beforeOneYear + weekCode + "' statistic_timestamp, '" +
      nowDate + "' adddate , " +
