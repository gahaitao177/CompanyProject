...
object AppDayStats {
  ...
  val getLastTime = dateUtils.splitDate(dateUtils.getLastDay(date, -3))
  ...
  val monthCode = getTime("year").toString + getTime("month")
  val statisticTimestamp = getTime("year").toString + getTime("month") + getTime("day")
  ...
  //落地表名称
  val userTable = "tb_app_users_stats_day"
  val networkTable = "tb_app_network_stats_day"
  val timeTable = "tb_app_time_stats_day"
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("====================================== Today New User")
    println(allNewUser.count())
    ...
    val result = session
      .sql("select " + monthCode + " monthcode,"
