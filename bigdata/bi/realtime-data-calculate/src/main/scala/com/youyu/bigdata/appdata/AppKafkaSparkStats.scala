package com.youyu.bigdata.appdata

import java.util.Date

import com.alibaba.fastjson.{JSON, JSONObject}
import com.youyu.bigdata.conf.ConfigurationManager
import com.youyu.bigdata.constant.Constants
import com.youyu.bigdata.utils.{DateUtils, HbaseUtil}
...
object AppKafkaSparkStats {
        ...
        partitionOfRecords.foreach(x => {
          ...
          try {
            ...
            if (!dayFlag) {
              ...
              //天活跃数据+1
              HbaseUtil.incrementColumnValues(userStatTable, userStatRowKey, "data", "all", 1L)
  ...
  def userInfoPkgDict(reports: DStream[JSONObject], scc: StreamingContext) = {
    ...
    //获取用户信息字典表中的RowKey
    val userPkgDictRowKey = HbaseUtil.getAllRowKey(user_pkg_dic)
      ...
      rdd.foreachPartition(partitionOfRecords => {
        ...
        partitionOfRecords.foreach(x => {
          ...
          try {
            ...
            if (!flag) {
              //判断当前rowKey在表中是否已经存在
              val isNotExistKey = HbaseUtil.isNotExistKey(userDictTable, userDictRowKey)

              if (isNotExistKey) {
                val columns = Array("app_name", "app_platform")
                val values = Array(appName, platform)
                HbaseUtil.addRow(userDictTable, userDictRowKey, "info", columns, values)
  ...
  def newUserCount(reports: DStream[JSONObject], scc: StreamingContext) = {
    ...
    //获取用户信息表中的RowKey
    val userInfoRowKeyMap = HbaseUtil.getAllRowKey(new_user_info_day)
        ...
        partitionOfRecords.foreach(x => {
          ...
          try {
            ...
            if (!flag) {
              ...
              val isNotExistKey = HbaseUtil.isNotExistKey(userInfoTable, userInfoRowKey)

              if (isNotExistKey) {
                //将信息插入到用户信息表中
                val columns = Array("client_id", "reg_date", "app_version", "app_channel")
                val values = Array(clientId, reportTime, version, channel)
                HbaseUtil.addRow(userInfoTable, userInfoRowKey, "info", columns, values)
          ...
          try {
            for (i <- 0 until starts.toArray().length) {
              ...
              if (startDate.equals(reportDate)) {
                ...
                HbaseUtil.incrementColumnValues(startsTable, startsRowKey, "data", startHour, 1)
                HbaseUtil.incrementColumnValues(startsTable, startsRowKey, "data", "all", 1)
              }
            }
          } catch {
            case e: Exception =>
